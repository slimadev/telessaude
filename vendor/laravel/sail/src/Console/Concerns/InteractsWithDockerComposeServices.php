...
trait InteractsWithDockerComposeServices
{
    ...
    protected function buildDockerCompose(array $services)
    {
        ...
        // Adds the new services as dependencies of the telessaude service...
        if (! array_key_exists('telessaude', $compose['services'])) {
            $this->warn('Couldn\'t find the telessaude service. Make sure you add ['.implode(',', $services).'] to the depends_on config.');
        } else {
            $compose['services']['telessaude']['depends_on'] = collect($compose['services']['telessaude']['depends_on'] ?? [])
                ->merge($services)
                ->unique()
                ->values()
                ->all();
